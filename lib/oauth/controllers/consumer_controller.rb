  ...
  module Controllers
    module ConsumerController
      ...
      def show
        unless @token
          if @consumer.ancestors.include?(Oauth2Token)
            redirect_to @consumer.authorize_url("#{callback2_oauth_consumer_url(params[:id])}#{callback2_querystring}")
          else
            @request_token = @consumer.get_request_token("#{callback_oauth_consumer_url(params[:id])}#{callback2_querystring}")
      ...
      def callback2_querystring
        return request.query_string.blank? ? '' : "?#{request.query_string}"
      end
