...
module OAuth
  module Controllers
    ...
    module ProviderController
      ...
      def request_token
        @token = current_oauth_application.create_request_token params
        if @token
      ...
      def token
        @oauth_application = OauthApplication.find_by_key! params[:client_id]
        if @oauth_application.secret != params[:client_secret]
      ...
      def revoke
        ...
        if @token
          @token.invalidate!
          flash[:notice] = "You've revoked the token for #{@token.oauth_application.name}"
        end
        ...
        unless @token.invalidated?
          if request.post?
            if user_authorizes_token?
              ...
              callback_url  = @token.oob? ? @token.oauth_application.callback_url : @token.callback_url
      ...
      def oauth2_token_authorization_code
        @verification_code =  @oauth_application.oauth2_verifiers.find_by_token params[:code]
      ...
      def oauth2_token_password
        ...
        @token = Oauth2Token.create :oauth_application=>@oauth_application, :user=>@user, :scope=>params[:scope]
        render :json=>@token
      end
      ...
      # autonomous authorization which creates a token for oauth_applications user
      def oauth2_token_client_credentials
        @token = Oauth2Token.create :oauth_application=>@oauth_application, :user=>@oauth_application.user, :scope=>params[:scope]
        render :json=>@token
      end
