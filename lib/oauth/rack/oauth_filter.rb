      ...
      def call(env)
        ...
        if token_string = oauth2_token(request)
          if token = Oauth2Token.first(:conditions => ['invalidated_at IS NULL AND authorized_at IS NOT NULL and token = ?', token_string])
            ...
            oauth_application = OauthApplication.find_by_key(request_proxy.consumer_key)
            env["oauth.oauth_application_candidate"] = oauth_application

            # Store this temporarily in oauth_application object for use in request token generation
            oauth_application.token_callback_url = request_proxy.oauth_callback if request_proxy.oauth_callback
            oauth_token = nil

            if request_proxy.token
              oauth_token = oauth_application.tokens.first(:conditions => { :token => request_proxy.token })
            ...
            [(oauth_token.nil? ? nil : oauth_token.secret), (oauth_application.nil? ? nil : oauth_application.secret)]
        end
          ...
          env["oauth.oauth_application"] = env["oauth.oauth_application_candidate"]
          env["oauth.version"] = 1

        end
        env["oauth.strategies"] = strategies unless strategies.empty?
        env["oauth.oauth_application_candidate"] = nil
