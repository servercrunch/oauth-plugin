...
Add the following lines to your user model:
  ...
  has_many :tokens, :class_name => "OauthToken", :order => "authorized_at desc", :include => [:oauth_application]
...
=== Changes in oauth_application.rb
...
Then change the create_request_token method to the following:
  ...
  def create_request_token
    RequestToken.create :oauth_application => self, :callback_url => token_callback_url
  end
    ...
    unless @token.invalidated?
      if request.post?
        if params[:authorize] == '1'
          ...
          if @token.oauth10?
            @redirect_url = params[:oauth_callback] || @token.oauth_application.callback_url
          else
            @redirect_url = @token.oob? ? @token.oauth_application.callback_url : @token.callback_url
          end
...
* current_oauth_application - for accessing information about which consumer is currently accessing your request

You could add application specific information to the OauthToken and OauthApplication model for such things as object level access control, billing, expiry etc. Be creative and you can create some really cool applications here.
