...
describe OauthController, "getting a request token" do
  ...
  before(:each) do
    ...
    @oauth_application.stub!(:create_request_token).and_return(@request_token)
    @oauth_application.stub!(:token_callback_url=)
  end
  ...
  it "should query for oauth_application" do
    OauthApplication.should_receive(:find_by_key).with('key').and_return(@oauth_application)
    do_get
  end
  
  it "should request token from oauth_application" do
    @oauth_application.should_receive(:create_request_token).and_return(@request_token)
    do_get
  end
  ...
  it "should not set token_callback_url" do
    @oauth_application.should_not_receive(:token_callback_url=).with(nil)
    do_get
  end
end
...
describe OauthController, "getting a request token passing a oauth_callback url" do
  ...
  it "should set token_callback_url with received oauth_callback" do
    @oauth_application.should_receive(:token_callback_url=).with("http://test.com/alternative_callback")
    do_get
  end
end
...
describe OauthController, "token authorization" do
  ...
  def do_post_with_no_application_callback
    @request_token.should_receive(:authorize!).with(@user)
    @oauth_application.stub!(:callback_url).and_return(nil)
    post :authorize, :oauth_token => @request_token.token, :authorize => "1"
  end
...
describe OauthController, "getting an access token" do
  ...
  it "should query for oauth_application" do
    OauthApplication.should_receive(:find_token).with(@request_token.token).and_return(@request_token)
    do_get
  end
...
describe OauthorizedController, " access control" do
  ...
  it "should have access_token set up correctly" do
    ...
    @access_token.oauth_application.should == @oauth_application
  end
  ...
  it "should allow oauth when using login_or_oauth_required" do
    ...
    OauthApplication.should_receive(:find_token).with(@access_token.token).and_return(@access_token)
    get :both
    controller.send(:current_token).should == @access_token
    controller.send(:current_token).is_a?(AccessToken).should == true 
    controller.send(:current_user).should == @user
    controller.send(:current_oauth_application).should == @oauth_application
