...
class OauthControllerRequestTokenTest < ActionController::TestCase
  ...
  def setup
    ...
    @oauth_application.stubs(:create_request_token).returns(@request_token)
  end
  ...
  def test_should_query_for_oauth_application
    OauthApplication.expects(:find_by_key).with('key').returns(@oauth_application)
    do_get
  end
  
  def test_should_request_token_from_oauth_application
    @oauth_application.expects(:create_request_token).returns(@request_token)
    do_get
  end
...
class OauthControllerTokenAuthorizationTest < ActionController::TestCase
  ...
  def do_post_with_no_application_callback
    @request_token.expects(:authorize!).with(@user)
    @oauth_application.stubs(:callback_url).returns(nil)
    post :authorize, :oauth_token => @request_token.token, :authorize=>"1"
  end
...
class OauthControllerGetAccessTokenTest < ActionController::TestCase
  ...
  def test_should_query_for_oauth_application
    OauthApplication.expects(:find_token).with(@request_token.token).returns(@request_token)
    do_get
  end
...
class OauthControllerAccessControlTest < ActionController::TestCase
  ...
  def test_should__have_access_token_set_up_correctly
    ...
    assert_equal @oauth_application, @access_token.oauth_application
  end
  ...
  def test_should_allow_oauth_when_using_login_or_oauth_required
    ...
    OauthApplication.expects(:find_token).with(@access_token.token).returns(@access_token)
    get :both
    assert_equal @access_token, @controller.send(:current_token)
    assert @controller.send(:current_token).is_a?(AccessToken)
    assert_equal @user, @controller.send(:current_user)
    assert_equal @oauth_application, @controller.send(:current_oauth_application)
